...
from db_setup import Base, InstrumentCategory, Instrument, User
...
# querying for it
categories = session.query(InstrumentCategory).order_by(
...
# if a user is not logged in and tries to add, edit, or delete
# they must be redirected
def redirectToLogin():
    return redirect(url_for('login', error='Please log in to continue.'))


# function to get the current logged in users id from database
def getUserId():
    username = login_session.get('username')
    return session.query(User).filter_by(username=username).first()


# function to check if logged in user is already in database
def isUserRegisterd(username):
    return session.query(User).filter_by(username=username).first()
...
def gconnect():
    ...
    if not isUserRegisterd(login_session['username']):
...
def gdisconnect():
    ...
    url += 'oauth2/revoke?token=%s' % login_session.get('access_token')
    print url
    h = httplib2.Http()
...
def addCategory():
    ...
    if request.method == 'GET':
        return render_template('categoryadd.html', categories=categories,
                               username=login_session.get('username'))
    else:
...
def editCategory(category_id):
    ...
    if user_id.id != category.user_id:
        flash("You cannot edit categories that aren't yours")
        return redirect(url_for('displayHome'))
...
def deleteCategory(category_id):
    ...
    # get the category
    category = getCategory(category_id)
    instruments = session.query(Instrument).filter_by(category_id=category.id).all()

    # check if the user owns the category
    user_id = getUserId()
    if user_id.id != category.user_id:
        flash("You cannot delete categories that aren't yours")
        return redirect(url_for('displayHome'))
...
def addInstrument(category_id):
    ...
    if not category:
        ...
        if request.method == 'GET':
            return render_template('instrumentadd.html', categories=categories,
                                   category=category,
                                   username=login_session.get('username'))
        else:
            ...
            user_id = getUserId()
            new_instrument = Instrument(name=name, description=description,
            ...
            return redirect(url_for('displayInstrument',
                                    category_id=category_id,
                                    instrument_id=new_instrument.id))


# handler to edit and existing instrument
@app.route('/category/<int:category_id>/<int:instrument_id>/edit/',
           methods=['GET', 'POST'])
def editInstrument(category_id, instrument_id):
    ...
    # check if the user owns the instrument
    user_id = getUserId()
    if user_id.id != instrument.user_id:
        flash("You cannot edit instruments that aren't yours")
        return redirect(url_for('displayHome'))
    ...
    if not (category and instrument):
        ...
        if request.method == 'GET':
            return render_template('instrumentedit.html',
                                   categories=categories, category=category,
...
@app.route('/category/<int:category_id>/<int:instrument_id>/delete/',
           methods=['GET', 'POST'])
def deleteInstrument(category_id, instrument_id):
    ...
    user_id = getUserId()
    if user_id.id != instrument.user_id:
        flash("You cannot delete instruments that aren't yours")
        return redirect(url_for('displayHome'))
    ...
    if not (category and instrument):
        ...
        # check what HTTP method is being used
        if request.method == 'GET':
            return render_template('instrumentdelete.html',
                                   categories=categories, category=category,
                                   instrument=instrument,
                                   username=login_session.get('username'))
        else:
...
def supplyCategoryJSON(category_id):
    ...
    instruments = session.query(Instrument).filter_by(
                  category_id=category_id).all()
    return jsonify(Instruments=[instrument.serialize for instrument in
                   instruments])
